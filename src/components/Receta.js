  ...
  const mostrarIngredientes = (informacion) => {
    ...
    for (let i = 1; i < 16; i++) {
      if (informacion[`strIngredient${i}`]) {
        ingredientes.push(
          <li>{informacion[`strIngredient${i}`]} {informacion[`strMesaure${i}`]}</li>
        )
      }
    }
  ...
  return (
    <div className="col-md-4 mb-3">
      <div className="card">
        ...
        <img className="img-top" src={receta.strDrinkThumb} alt={`Imagen de ${receta.strDrink}`} />

        <div className="card-body">
          <button
            type="button"
            className="btn btn-button btn-primary"
            onClick={() => {
